...
class TrafficBlaster:
    def __init__(self, sessionConstructorFunction=None):
        # OpenVPN Settings
        self.p = pp()
        self.log = Logger()  # <--- Replace me with something better please
        self.OPENVPN_EXE_FULL_PATH = "C:\\Program Files\\OpenVPN\\bin\\openvpn.exe"
        self.OPENVPN_CONFIG_FOLDER_PATH = "C:/Users/justi/PycharmProjects/Traffic_Blaster/ovpn configs"
        self.OPENVPN_CREDENTIALS_PATH = "C:/Users/justi/Documents/ovpncreds.txt"
        self.OVPN_CONNECT_RETRY_MAX = 3  # this is probably faster at 0, but causes more issues spinning up and destroying connections
        self.OVPN_CONNECT_TIMEOUT = 30  # probably crank this down to like 10 and see what happens, it does not respect me
        self.OVPN_POLL_TIMEOUT = 60  # not sure exactly, 30 was too low though, maybe
        self.CREATE_VPN_TIMEOUT_SECONDS = self.OVPN_CONNECT_TIMEOUT + self.OVPN_POLL_TIMEOUT + 5  #
        self.OVPN_RESOLVE_RETRY = 0  # another futile attempt to keep the process from completely hanging
        self.OVPN_PING_RESTART = 60  # keepalive restart
        self.OVPN_PING = 10  # keepalive

        # Traffic Blaster Settings
        self.WAIT_AFTER_VPN_CREATE_SECONDS = 5  # Potentially effective at preventing windows getting confused. Slows testing
        self.NUM_CONCURRENT_VPN_PROCS = 7  # Most stable at 1 core / connection. works fine over, kinda
        self.NUM_PROCS_RESERVED_FOR_SWAPPING = 0  # Basically hardcoded right now. Would be nice to implement this functionality
        self.RECYCLE_SESSION_TIMEOUT_SECONDS = 60 * 5
        self.LIMIT_PER_LOCATION = -1  # This would be neat
        # runtime variables
        self.response_dictionary = {"Initialization Sequence Completed": 1,
        ...
        if sessionConstructorFunction is None:
            self.session_constructor_function = self.default_session_constructor_function
        else:
            self.session_constructor_function = sessionConstructorFunction
